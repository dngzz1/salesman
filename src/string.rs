pub fn get_string_order(points: &[(f32, f32)], salesmen_capacities: &[usize]) -> Vec<usize> {
    let mut clustered_order = crate::cluster::cluster_order(&points, &salesmen_capacities);
    for i in 0..salesmen_capacities.len() {
        ...
        let slice_order = crate::anneal::shortest_path_order(&filtered_points, 2, false);
